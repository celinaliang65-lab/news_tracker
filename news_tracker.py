...
def get_monthly_revenue(sid):
    try:
        ...
        # 過濾掉尚未正式公布的月份（每月10日後才視為公布）
        filtered = []
        for d in data:
            rev_year = int(d["date"][:4])
            rev_month = int(d["date"][5:7])
            # 該月營收預計於次月10日公布
            pub_month = rev_month % 12 + 1
            pub_year = rev_year if rev_month < 12 else rev_year + 1
            expected_pub = datetime(pub_year, pub_month, 10, tzinfo=timezone(timedelta(hours=8)))
            if now_tw >= expected_pub:
                filtered.append(d)
